#!/usr/bin/env python3
"""Regenerate docs/adr/INDEX.md from ADR frontmatter, or look up what binds a path.

Run manually: uv run python -m scripts.adr.generate_index
...
# are checked against it. `REPO_ROOT` is read only on the `__main__` line, where the
# repo-relative `ADR_DIR` is joined to it, so every function below requires the root
...
# whoever is editing a path one of them binds. Superseded is neither: it is history, and
# "supersede, don't edit" means a stale path in one could not be fixed anyway.
...
# The paths where a breach of a decision would be authored, so it reaches whoever edits
# one of them. Entries are exact file paths or directory prefixes ending in `/`, never
# globs.
SCOPE_FIELD = "scope"

# Replaced by `scope`. Named here so a stale field fails rather than being ignored, which
# is what would otherwise let a half-finished migration pass in both directions.
TAGS_FIELD = "tags"

INDEX_HEADER = (
    "<!-- Auto-generated by `uv run python -m scripts.adr.generate_index` — do not edit "
    "manually. -->\n"
    "\n"
    "# ADR Index\n"
)
...
def parse_adr(content: str) -> tuple[dict[str, str | list[str]], str, list[str]]:
    ...
    # decision binds no path. An optional field's absence would carry no more than the
    # missing tags it replaced: unfilled and deliberate look identical.
...
def scope_problems(entries: list[str], repo_root: Path) -> list[str]:
    ...
    Separate from ``parse_adr`` because this is the one rule needing the filesystem, and
    keeping that out of the parser is what lets every other rule be checked against a
    string.
    """
    problems = []
    for entry in entries:
        # A glob is the natural thing to reach for and the error for one would otherwise
        # be "nothing matches", which reads as a wrong path rather than as syntax that
        # was never supported.
        ...
        if not target.exists():
            problems.append(
                f"scopes `{entry}`, which nothing matches; correct it, or drop it where the "
                "code it named is gone for good"
            )
...
def generate(adr_dir: Path, repo_root: Path) -> int:
    ...
    Neither directory is defaulted: `main` supplies the module's anchors, and a test
    that forgot its fixture would otherwise rewrite this repository's own index.
    """
    ...
    for path in sorted(adr_dir.iterdir()):
        ...
        if not problems and fields["status"] in BINDING_STATUSES:
            # parse_adr already added a problem above if scope isn't a list, so
            # reaching here means it is one.
            scope = fields[SCOPE_FIELD]
            assert isinstance(scope, list)
            problems = scope_problems(scope, repo_root)
        if problems:
...
def report_scoped_to(paths: list[str], adr_dir: Path, repo_root: Path) -> int:
    ...
    which decisions bind the file in front of me — and so reaches them through the work
    rather than through a search they had to think to run. It reports Archived decisions
