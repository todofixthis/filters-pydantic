...
Stdlib `unittest`, collected and run by pytest: `uv run pytest scripts`.

Every function here requires its directories, so these tests pass a fixture root and
never chdir; there is no argumentless call that could rewrite the real docs/adr/INDEX.md.
The one test that does chdir asserts precisely that — that moving cwd cannot redirect
the module's anchors — and reads the constants rather than calling anything.
"""
...
TRIGGER = "A second filter chain integration joins the package."
...
class ParseAdrTests(unittest.TestCase):
    ...
    def problems(self, content: str) -> list[str]:
    ...
    def test_propagates_frontmatter_problems(self) -> None:
    ...
    def test_reports_a_wrapped_frontmatter_value(self) -> None:
    ...
    def test_a_horizontal_rule_does_not_extend_the_frontmatter(self) -> None:
    ...
    def test_the_first_heading_wins(self) -> None:
    ...
    def test_requires_the_field_each_status_owns(self) -> None:
        ...
        for status, field in STATUS_FIELDS.items():
            with self.subTest(status=status):
                self.assertIn(
                    ...
                    self.problems(adr(status=status)),
                )

    def test_rejects_an_empty_value_for_a_status_field(self) -> None:
    ...
    def test_accepts_a_status_carrying_its_own_field(self) -> None:
        ...
        self.assertEqual(
            self.problems(adr(status="Superseded", **{"superseded-by": "12"})), []
        )
    ...
    def test_rejects_a_missing_scope(self) -> None:
        ...
        self.assertIn(
            ...
            # A dict[str, None] unpacked as **fields can't statically rule out
            # colliding with `title: str`, so mypy checks it against that too.
            self.problems(adr(**{SCOPE_FIELD: None})),  # type: ignore[arg-type]
        )
...
class ScopeProblemsTests(unittest.TestCase):
    ...
    def test_accepts_entries_that_resolve(self) -> None:
        ...
        self.assertEqual(
            scope_problems(["scripts/versions.py", "scripts/"], self.root), []
        )
...
class RelativeToRepoTests(unittest.TestCase):
    ...
    def test_leaves_a_relative_path_alone(self) -> None:
        """Scope entries are written repo-relative, so that form is already the answer."""
        self.assertEqual(
            relative_to_repo("scripts/ci/versions.py", self.root),
            "scripts/ci/versions.py",
        )

    def test_converts_an_absolute_path_inside_the_repo(self) -> None:
        ...
        self.assertEqual(
            relative_to_repo(absolute, self.root), "scripts/ci/versions.py"
        )

    def test_rejects_a_path_outside_the_repo(self) -> None:
...
class GenerateTests(AdrDirTestCase):
    ...
    def test_writes_a_row_for_each_accepted_adr(self) -> None:
    ...
    def test_excludes_hidden_statuses_but_keeps_their_neighbours(self) -> None:
        ...
        for status in HIDDEN_STATUSES:
            with self.subTest(status=status):
                ...
                self.write(
                    "002-hidden.md", adr(status=status, **{STATUS_FIELDS[status]: "12"})
                )
                code, out, _ = self.run_generate()
                self.assertEqual(code, 0)
    ...
    def test_orders_rows_by_file_number(self) -> None:
    ...
    def test_ignores_the_index_and_dot_files(self) -> None:
    ...
    def test_says_so_when_there_are_no_adrs(self) -> None:
    ...
    def test_carries_a_revisit_trigger_into_its_own_column(self) -> None:
    ...
    def test_omits_a_discharged_trigger_from_its_column(self) -> None:
    ...
    def test_leaves_the_scope_cell_empty_for_a_decision_binding_no_path(self) -> None:
    ...
    def test_lists_every_scope_entry_in_one_cell(self) -> None:
        """A decision binding several paths must show all of them, not the first."""
        self.write_scoped("CHANGELOG.md")
        self.write(
            "001-first.md", adr(**{SCOPE_FIELD: f"[{SCOPED_FILE}, CHANGELOG.md]"})
        )
        self.run_generate()
...
class GenerateFailureTests(AdrDirTestCase):
    ...
    def test_rejects_a_file_that_is_not_an_adr(self) -> None:
    ...
    def test_rejects_an_adr_whose_filename_breaks_the_convention(self) -> None:
    ...
    def test_reports_a_problem_against_the_file_that_holds_it(self) -> None:
    ...
    def test_reports_every_bad_file(self) -> None:
    ...
    def test_a_valid_sibling_does_not_rescue_the_run(self) -> None:
    ...
    def test_rejects_a_scope_naming_something_that_is_gone(self) -> None:
        ...
        self.assert_rejected(
            "001-first.md scopes `scripts/gone.py`, which nothing matches"
        )

    def test_checks_the_scope_of_an_archived_adr(self) -> None:
    ...
    def test_leaves_the_scope_of_a_superseded_adr_alone(self) -> None:
...
class ReportScopedToTests(AdrDirTestCase):
    ...
    def test_warns_that_an_unreadable_adr_binds_nothing(self) -> None:
        ...
        self.assertIn(
            "001-bad.md could not be read, so it binds nothing here", err.getvalue()
        )

    def test_names_the_decision_binding_the_path(self) -> None:
    ...
    def test_omits_a_superseded_decision(self) -> None:
    ...
    def test_reports_every_decision_binding_the_path(self) -> None:
    ...
    def test_finds_a_decision_from_an_absolute_path(self) -> None:
    ...
    def test_matches_any_of_several_paths(self) -> None:
...
class ArgumentTests(unittest.TestCase):
    ...
    def test_rejects_for_with_no_path(self) -> None:
...
class RepoRootTests(unittest.TestCase):
    ...
    def test_chdir_cannot_redirect_the_anchor(self) -> None:
    ...
    def test_the_root_is_the_repo_rather_than_the_package_inside_it(self) -> None:
