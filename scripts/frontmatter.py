"""Parse the flat YAML frontmatter used by ADRs.

Stdlib-only by design (ADR 003): frontmatter here is a flat key/value block, so a
line parser suffices and the repo needs no PyYAML.
...
def parse_frontmatter(block: str) -> tuple[dict[str, str | list[str]], list[str]]:
    ...
    become list[str]; scalars stay str.
    """
        ...
        if line[:1].isspace():
            ...
            if sequence_key is not None and stripped.startswith("- "):
                ...
                if not isinstance(sequence, list):
                    sequence = []
                    fields[sequence_key] = sequence
                sequence.append(stripped[2:].strip())
            else:
                problems.append(
                    f"continued onto another line; wrap it onto one: {stripped!r}"
                )
            continue
        sequence_key = None
        if ":" not in line:
