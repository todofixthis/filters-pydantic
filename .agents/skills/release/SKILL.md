---
name: release
description: Use when preparing or publishing a new release of phx-filters-pydantic — covers release notes, version bump, build, PyPI upload, and GitHub release creation
---
# Release
...
uv version <version>
```
This updates `pyproject.toml` and re-locks `uv.lock` in one step. Commit both files and push.
...
uv build
```
Sync first — merging the release PR may have brought in dependency changes. Artefacts land in `dist/`.
...
sha256sum dist/phx_filters_pydantic-* >> release-<version>.md
```
...
for f in dist/phx_filters_pydantic-*; do gpg --local-user "$GPG_KEY" --detach-sign "$f"; done
# Creates dist/phx_filters_pydantic-*.sig alongside each artefact
```
...
gh release create <version> dist/* \
  --title "Pydantic Filters v<version>" \
...
```bash
gh issue close <number> --comment "Implemented in [v<version>](https://github.com/todofixthis/filters-pydantic/releases/tag/<version>)."
```
...
> 3. Verify: `gpg --verify phx_filters_pydantic-<version>-py3-none-any.whl.sig phx_filters_pydantic-<version>-py3-none-any.whl`
>
